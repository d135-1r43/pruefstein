 ...
 * comfortably under the limit. Expressions are taken from the catalog rather
...
class CatalogExpressionTest
{
	...
	{
		return evaluator.evaluate(json, ComplianceCatalog.check(checkKey).expression());
	}
	...
	{
		assertTrue(evaluate("a9.auto-login", loginWindow(1, 9, 0)));
		assertTrue(evaluate("a9.guest-account", loginWindow(1, 9, 0)));
	}

	@Test
	void aSettingThatIsOnFails() throws Exception
	{
		assertFalse(evaluate("a9.auto-login", loginWindow(1, 9, 1)));
		assertFalse(evaluate("a9.guest-account", loginWindow(1, 9, 1)));
	}
	...
	void aFileThatExistsButCouldNotBeReadFails() throws Exception
	{
		...
		assertFalse(evaluate("a9.auto-login", loginWindow(1, 0, 0)));
		assertFalse(evaluate("a9.guest-account", loginWindow(1, 0, 0)));
	}
	...
	void aMissingFilePasses() throws Exception
	{
		...
		assertTrue(evaluate("a9.auto-login", loginWindow(0, 0, 0)));
		assertTrue(evaluate("a9.guest-account", loginWindow(0, 0, 0)));
	}
	...
	{
		assertTrue(evaluate("a9.screen-lock-timeout", idle(1, "300", "300")));
		assertTrue(evaluate("a9.screen-lock-timeout", idle(2, "60", "300")));
	}

	@Test
	void aTimeoutOverThePolicyFails() throws Exception
	{
		assertFalse(evaluate("a9.screen-lock-timeout", idle(1, "600", "600")));
	}
	...
	void oneAccountOverThePolicyFailsTheMachine() throws Exception
	{
		...
		// to be tidiest
		assertFalse(evaluate("a9.screen-lock-timeout", idle(2, "60", "600")));
	}
	...
	void aScreenSaverThatNeverStartsFails() throws Exception
	{
		...
		assertFalse(evaluate("a9.screen-lock-timeout", idle(1, "0", "0")));
	...
	void noTimeoutConfiguredAnywhereFails() throws Exception
	{
		...
		assertFalse(evaluator.evaluate(
			...
			ComplianceCatalog.check("a9.screen-lock-timeout").expression()));
	}
}
